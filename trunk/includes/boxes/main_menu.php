...
<?php
  ...
  $info_box_contents[] = array('text' => tep_draw_form('menu_100', tep_href_link(FILENAME_DEFAULT)) . tep_create_parameters(array('mPath'=>'100'), array('period'), 'hidden_field') . tep_href_submit(BOX_MAINMENU_HOME, 'submitLinkInfoBoxContents', ($_POST['mPath']=='100'||$_POST['mPath']==''?'style="font-weight:bold;cursor:default" disabled':'')) . '</form><br>' .
                                         ...
                                         (substr($_POST['mPath'], 0, 1) == '4'?'<br>' .
                                           ...
                                           tep_draw_form('menu_430', tep_href_link(FILENAME_ADMINISTRATION)) . tep_create_parameters(array('mPath'=>'430'), null, 'hidden_field') . '&nbsp;&nbsp;&nbsp;' . tep_href_submit(BOX_MAINMENU_ADMINISTRATION_PROJECTS, 'submitLinkInfoBoxContents', ($_SESSION['employee']->profile->right['administration']?($_POST['mPath']=='430'?'style="font-weight:bold;cursor:default" disabled':''):'style="cursor:default" disabled')) . '</form>' .
