...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="entryListing">
  ...
  if ($database->num_rows($ticket_query) > 0) {
    ...
    while ($ticket_result = $database->fetch_array($ticket_query, MYSQL_NUM)) { ?>
      <tr onmouseover="this.className='selectableRowMouseOver'" onmouseout="this.className='entryListing-<?php echo $odd_or_even; ?>'" class="entryListing-<?php echo $odd_or_even; ?>" onclick="closeWindow('<?php echo $ticket_result[0]; ?>', '<?php echo rawurlencode($ticket_result[1]); ?>');">
